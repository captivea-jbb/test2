...
class WebsiteSale(WebsiteSale):
    ...
    def update_cart_popup(self):
        order = request.website.sale_get_order()
        _logger.warning("update_cart_popup LAUNCHED")
    ...
    def move_to_cart(self, line_id=None):
        if line_id:
            ...
            _logger.warning("move_to_cart LAUNCHED")
    ...
    def add_all_to_cart(self, line_id=None):
        ...
        for data in line_ids:
            ...
            _logger.warning("add_all_to_cart LAUNCHED")
        return True
    ...
    def qv_update_my_cart(self, set_qty=0, **kw):
        if 'prod_id' in kw and 'qty_val' in kw:
            ...
            _logger.warning("qv_update_my_cart LAUNCHED")
        return
    ...
    # ADDED BY CAPTIVEA - 05/07/2021 - BEGIN

    # def get_lot_info(self, product_id, request):
    #     st = "Approx. 16-20 Weeks"
    #     lots = request.env['x_ctr_lot'].sudo().search([('x_product_id','=',product_id)])
    #     product = request.env['product.product'].sudo().browse(product_id)
    #     _logger.warning("TEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEST" + str(product))
    #     # qty = product.sudo().qty_available
    #     qty = request.env['product.product'].with_user(SUPERUSER_ID).browse(product_id).qty_available
    #     lot_date = False
    #     lot_name = False
    #     for lot in lots:
    #         if not lot.x_lot_id:
    ...
    #                     lot_name = lot.x_name + " (" + str(remaining) + " remaining)"
    #                     break
    ...
    def cart_update(self, product_id, add_qty=1, set_qty=0, **kw):
        ...
        # lot_info = self.get_lot_info(product_id=product_id, request=request)
        # _logger.warning("________TEST________: " + str(lot_info))
